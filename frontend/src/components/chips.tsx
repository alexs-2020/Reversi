...
function Chip({ color, onClick }: ChipsProps) {
  ...
  return (
    <div onClick={handleClick} className={isClicked ? 'chip-shadow' : ''}>
      <svg xmlns="http://www.w3.org/2000/svg" width="5.19vh" height="5.19vh" viewBox="0 0 100 100">
        <circle cx="50" cy="50" r="45" fill="#07A5C3" />
        <circle cx="50" cy="50" r="40" fill="#D4F4FE" />
        <circle cx="50" cy="50" r="35" fill={color} />
        <circle cx="50" cy="50" r="40" fill="transparent" stroke="#fff" strokeWidth="5" />
      </svg>
    </div>
  );
}
