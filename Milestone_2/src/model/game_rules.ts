...
class GameRules {
    ...
    validMove: {move: Move|null, valid_direction:number, positions: { row: number, col: number }[]|null}
...
isLegalMove(move: Move, valid_placement: { move: Move,valid_direction:number,  positions: { row: number, col: number }[] }[]): boolean {
    ...
    this.validMove = foundPlacement || { move: null, valid_direction:0,  positions: null };
    ...
    checkDirection(move: Move, direction: { row: number, col: number }, symbol: PlayerSymbol): {
        row: number,
        col: number
    }[] {
...
//check if player has any valid playable moves
    getValidPlacements(player: Player): { move: Move,valid_direction:number, positions: { row: number, col: number }[] }[] {
        ...
        for (let col = 0; col < this.board.board[0].length; col++) {
            if (this.board.board[row][col] == PlayerSymbol.Empty) {
                ...
                if (positions.length > this.valid_directions) {
                    valid_placements.push({ move: curr_mv,valid_direction: this.valid_directions, positions: positions });
                }
            }
        }
    }
