class QueryTemplate {
  constructor() {
    if (new.target === QueryTemplate) {
      throw new Error(
        "QueryTemplate is an abstract class and cannot be instantiated directly.",
      );
    }
    this.query = "";
    this.fields = [];
...
class PiecesQuery extends QueryTemplate {
  ...
  createQuery() {
    this.query =
      "CREATE TABLE IF NOT EXISTS pieces (id INT AUTO_INCREMENT PRIMARY KEY, board_id INT, position VARCHAR(5), color VARCHAR(100), FOREIGN KEY (board_id) REFERENCES board(id))";
    this.fields = [];
  }
}
class GameQuery extends QueryTemplate {
  ...
  createQuery() {
    this.query =
      "CREATE TABLE IF NOT EXISTS game_sessions (id INT AUTO_INCREMENT PRIMARY KEY, board_id INT, user_id INT, FOREIGN KEY (board_id) REFERENCES board(id), FOREIGN KEY (user_id) REFERENCES users(id))";
    this.fields = [];
  }
}
