...
const io = new Server(server_app, {  // Using the Server class to create the instance
    cors: {
...
io.on('connection', (socket) => {
    ...
    socket.on('addPlayer', (username) => {
        ...
        for (let id in players) {
            if (players[id].username === username && players[id].id !== socket.id) {
                console.log(`Username already in use. Disconnecting old session: ${username}`);
                players[id].socket.disconnect(true); // Forcefully disconnect the old session
                delete players[id]; // Remove the old player from the list
                break;
            }
        }
    ...
    socket.on('startGame', (opponentId) => {
        if (players[opponentId]) {
            ...
            players[socket.id].socket.emit('gameStarted', { gameId, opponent: players[opponentId].username });
            players[opponentId].socket.emit('gameStarted', { gameId, opponent: players[socket.id].username });
        }
    });
    ...
    socket.on('disconnect', () => {
        ...
        Object.keys(games).forEach(gameId => {
            if (games[gameId].player1.id === socket.id || games[gameId].player2.id === socket.id) {
                delete games[gameId];
...
app.post('/signup', (req, res) => {
  ...
  const checkUsernameSql = userQueryBuilder.createQuery(username).checkUsername().build();

  db.query(checkUsernameSql.query, checkUsernameSql.fields, handleUsernameCheck(username, password, res));
});
...
function handlePasswordHashing(username, res) {
  return (err, hash) => {
   ...
   const { UserQueryBuilder, BoardQueryBuilder } = require('./routes/DbBuilder');
    ...
    const newUser = userQueryBuilder.setUsername(username).setPassword(hash).signUp().build();
