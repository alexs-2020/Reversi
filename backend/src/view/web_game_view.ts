...
class WebGameView extends GameView {
  ...
  showPlayerScores(player1: Player, player2: Player): [string, string[]] {
    return [
      'showPlayerScores', [
        this.logAndReturn('showPlayerScores', `\nPlayer ${symbolToStr[player1.symbol]} current score: ${player1.getScore()}`)[1],
        this.logAndReturn('showPlayerScores', `Player ${symbolToStr[player2.symbol]} current score: ${player2.getScore()}`)[1]
      ]
    ];
  }

  showCurrentPlayer(player: Player): [string, string] {
    return this.logAndReturn('showCurrentPlayer', `current player: Player ${symbolToStr[player.symbol]}`);
  }
  ...
  showPossibleMove(moves: Move[]): [string, string[]] {
    return [
      'showPossibleMove', moves.map((move, index) => this.logAndReturn('showPossibleMove', `Move ${index + 1}: Row ${move.row + 1}, Column ${move.column + 1}`)[1])
    ];
  }

  showWinner(player: Player): [string, string] {
    return this.logAndReturn('showWinner', `Player ${symbolToStr[player.symbol]} is the winner!`);
  }
